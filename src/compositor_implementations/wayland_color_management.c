...
static void wp_color_manager_destroy_resource(struct wl_resource *resource) {
...
static void wp_color_manager_destroy_request(struct wl_client *client,
                                             struct wl_resource *resource) {
  (void)client;
  wl_resource_destroy(resource);
}

static void wp_color_manager_get_output(struct wl_client *client,
                                        struct wl_resource *resource,
                                        uint32_t id,
...
static void wp_color_manager_get_surface(struct wl_client *client,
                                         struct wl_resource *resource,
                                         uint32_t id,
...
static void wp_color_manager_create_icc_creator(struct wl_client *client,
                                                struct wl_resource *resource,
                                                uint32_t id) {
  (void)client;
  (void)resource;
...
static void wp_color_manager_create_parametric_creator(
...
static void bind_color_manager(struct wl_client *client, void *data,
                               uint32_t version, uint32_t id) {
  struct wp_color_manager_impl *manager = data;
  struct wl_resource *resource = wl_resource_create(
      client, &wp_color_manager_v1_interface, (int)version, id);
  if (!resource) {
    wl_client_post_no_memory(client);
    return;
  }
  wl_resource_set_implementation(resource, &color_manager_impl, manager,
                                 wp_color_manager_destroy_resource);
}
#endif

static void bind_color_manager_macos(struct wl_client *client, void *data,
                                     uint32_t version, uint32_t id) {
  struct wp_color_manager_impl *manager = data;
  struct wl_resource *resource = wl_resource_create(
      client, &wp_color_manager_v1_interface, (int)version, id);
...
wp_color_manager_create(struct wl_display *display,
                        struct wl_output_impl *output) {
  struct wp_color_manager_impl *manager =
      calloc(1, sizeof(struct wp_color_manager_impl));
  if (!manager)
    return NULL;
...
#if TARGET_OS_IPHONE || TARGET_OS_SIMULATOR
  manager->global = wl_global_create(display, &wp_color_manager_v1_interface, 1,
                                     manager, bind_color_manager);
#else
  manager->global = wl_global_create(display, &wp_color_manager_v1_interface, 1,
                                     manager, bind_color_manager_macos);
#endif
  if (!manager->global) {
    free(manager);
    return NULL;
...
#endif
#else
  // iOS: HDR support depends on device, for now assume false or check specific
  // APIs
  return false;
#endif
}

CGColorSpaceRef create_colorspace_from_image_description(
    struct wp_image_description_impl *desc) {
  if (!desc)
    return NULL;
#if TARGET_OS_IPHONE || TARGET_OS_SIMULATOR
