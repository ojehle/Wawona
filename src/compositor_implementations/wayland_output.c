...
static void send_output_geometry(struct wl_resource *resource,
                                 struct wl_output_impl *output) {
  // Convert pixel dimensions to physical dimensions in millimeters
  // Using 96 DPI (standard desktop DPI) as a reasonable assumption for virtual
  // displays Formula: mm = (pixels / DPI) * 25.4
  const int32_t dpi = 96;
  int32_t physical_width_mm =
      (output->width * 254) /
      (dpi * 10); // (width / dpi) * 25.4, using integer math
  int32_t physical_height_mm = (output->height * 254) / (dpi * 10);

  // Ensure minimum size (at least 1mm) to avoid protocol violations
  if (physical_width_mm < 1)
    physical_width_mm = 1;
  if (physical_height_mm < 1)
...
static void send_output_mode(struct wl_resource *resource,
                             struct wl_output_impl *output) {
  // Send current mode WITHOUT PREFERRED flag to indicate arbitrary resolution
  // support. PREFERRED flag indicates a fixed preferred resolution, which
  // conflicts with arbitrary resolution support. For arbitrary resolutions, we
  // only send CURRENT to indicate this is the current mode, but clients can
  // create surfaces of any size.
  wl_output_send_mode(resource,
                      WL_OUTPUT_MODE_CURRENT, // Only CURRENT, not PREFERRED
                      output->width, output->height, output->refresh_rate);
}

static void destroy_output_resource(struct wl_resource *resource) {
...
static void bind_output(struct wl_client *client, void *data, uint32_t version,
  ...
  // Send mode with CURRENT flag only (no PREFERRED) to indicate arbitrary
  // resolution support The mode represents the current output size, but clients
  // can create surfaces of any size
...
struct wl_output_impl *wl_output_create(struct wl_display *display,
                                        int32_t width, int32_t height,
                                        int32_t scale, const char *name) {
  ...
  // scale, name, description, and done events needed for arbitrary resolution
  // support
  output->global =
      wl_global_create(display, &wl_output_interface, 4, output, bind_output);
  if (!output->global) {
    free(output);
    return NULL;
  }
...
void wl_output_destroy(struct wl_output_impl *output) {
  ...
  // but we should probably remove the link if we are destroying the output
  // while running. However, wl_resource_destroy handles unlinking if we set a
  // destructor. We didn't set a resource destructor that unlinks from
  // resource_list. We should iterate and destroy resources or rely on wayland
  // to clean up client resources.
...
void wl_output_update_size(struct wl_output_impl *output, int32_t width,
  ...
  wl_resource_for_each(resource, &output->resource_list) {
    ...
    if (scale_changed &&
        wl_resource_get_version(resource) >= WL_OUTPUT_SCALE_SINCE_VERSION) {
      wl_output_send_scale(resource, output->scale);
    }
