...
// CoreFoundation/IOSurface bindings
#[link(name = "IOSurface", kind = "framework")]
extern "C" {
    // Kept for future reference but currently verified in Bridge
    // fn IOSurfaceLookup(csid: u32) -> *mut std::ffi::c_void;
    ...
    fn request(
        ...
        match request {
            zwp_linux_buffer_params_v1::Request::Add { fd, plane_idx, offset, stride, modifier_hi, modifier_lo } => {
                ...
                let p = state.pending_dmabuf_params.entry(params_id).or_default();
            ...
            zwp_linux_buffer_params_v1::Request::Create { width, height, format, flags: _ } => {
                let params_id = resource.id().protocol_id();
                if let Some(p) = state.pending_dmabuf_params.remove(&params_id) {
