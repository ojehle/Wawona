...
// Note: These protocol headers don't exist yet - commented out until
// implemented #include "../compositor_implementations/wayland_idle_notifier.h"
...
- (BOOL)setupProtocols {
...
#endif
  xdg_wm_base_set_output_size(_compositor.xdg_wm_base,
                              (int32_t)initialFrame.size.width,
  ...
  // }
  _compositor.decoration_manager = NULL;
  ...
  // }
  _compositor.text_input_manager =
      NULL; // Set to NULL since protocol not implemented
  ...
  // if (wl_drm) {
  //   NSLog(@"   ✓ wl_drm protocol created (stub for macOS EGL
  //   compatibility)");
  // }
  ...
  // struct wp_presentation_impl *presentation =
  // wp_presentation_create(display); if (presentation) {
  //   NSLog(@"   ✓ Presentation time protocol created");
  // }

  // Color management protocol (for color operations and HDR support)
#if TARGET_OS_IPHONE || TARGET_OS_SIMULATOR
  // _compositor.color_manager = wp_color_manager_create(display,
  // _compositor.output); if (_compositor.color_manager) {
