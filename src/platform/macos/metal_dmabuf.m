...
metal_dmabuf_create_buffer(uint32_t width, uint32_t height, uint32_t format) {
...
id<MTLTexture> metal_dmabuf_get_texture(struct metal_dmabuf_buffer *buffer,
                                        id<MTLDevice> device) {
  if (!buffer || !buffer->iosurface || !device)
    return nil;
...
IOSurfaceRef metal_dmabuf_create_iosurface_from_data(void *data, uint32_t width,
                                                     uint32_t height,
                                                     uint32_t stride,
                                                     uint32_t format
  ...
  // Lock and copy data
  if (IOSurfaceLock(iosurface, 0, NULL) != 0) {
  ...
  // Always handle stride differences (source stride may differ from aligned
  // stride)
  uint8_t *src = (uint8_t *)data;
  uint8_t *dst = (uint8_t *)surfaceBase;
  uint32_t copyWidth =
      (stride < surfaceStride) ? stride : (uint32_t)surfaceStride;
...
int metal_dmabuf_get_fd(struct metal_dmabuf_buffer *buffer) {
  ...
  // This is a placeholder - real implementation would serialize IOSurface
  // properly
...
struct metal_dmabuf_buffer *metal_dmabuf_import(int fd, uint32_t width,
                                                uint32_t height,
                                                uint32_t format,
                                                uint32_t stride) {
  if (fd < 0)
  ...
  NSLog(@"✅ Imported IOSurface DMA-BUF buffer: %dx%d (ID: %llu)", width,
        height, surfaceID);
  return buffer;
}
